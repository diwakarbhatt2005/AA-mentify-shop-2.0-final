...
export async function verifyOtp(payload: VerifyOtpData) {
  const { status, body } = await postJson<ApiResponse<VerifyOtpResult> | ValidationError>(
    '/api/auth/verify-otp',
    payload
  )

  if (status === 200) return { ok: true, data: body as ApiResponse<VerifyOtpResult> }
